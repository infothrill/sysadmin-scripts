...
import os
def getMySqlCredsFromVpopmail(VPOPMAIL_DIR = '/home/vpopmail'):
    ...
    for line in cfgfile:
        if line.find("#") == -1: # non-comment line
            line = line.rstrip()
            credlist = line.split("|")
            return { "host": credlist[0], "port": credlist[1], "user": credlist[2], "passwd": credlist[3], "db": credlist[4] }
    return None
...
def main():
    ...
    fname = sys.argv[1]
    imapline = re.compile('^.*imapd(-ssl){0,1}: LOGIN, user=(.*), ip=\[::ffff:(\d+\.\d+\.\d+\.\d+)\], protocol=IMAP$') # IPv4 only
    t = Tail(fname)

    creds = getMySqlCredsFromVpopmail(VPOPMAIL_PATH)
    if creds is None:
        print "No mysql vpopmail credentials found"
        return 1

    for line in t:
        matchObj = imapline.match(line)
        if matchObj is not None:
            ip = matchObj.group(3)
            if ip != '127.0.0.1':
                t = long(time())
                sys.stdout.write("%s|%s|%s\n" % (ip, t, matchObj.group(2)))
                sys.stdout.flush()
                try:
                    # always reconnect, that's slow but relatively safe
                    dbh = MySQLdb.Connect(creds['host'], creds['user'], creds['passwd'], creds['db'])
                    cursor = dbh.cursor()
                    cursor.execute("replace into `relay` ( ip_addr, timestamp ) values ( '%s', '%s' )" % (str(ip), str(t)) )
                    cursor.close()
                except Exception, e:
                    sys.stderr.write("Failed updating relay table: %r\n" % repr(e))
                try:
                    # spawn in bg:
                    proc = subprocess.Popen( [os.path.join(VPOPMAIL_PATH, 'bin', 'clearopensmtp') ] )
                    proc.wait()
                except Exception, e:
                    sys.stderr.write("Failed running clearopensmtp: %r\n" % repr(e))
